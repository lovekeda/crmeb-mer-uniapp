							...
							<view class="menu acea-row" :id="'nav' + indexw" :style="[gridColumns]">
								<view :style="[titleColor]" class="item" :class="number===1?'four':number===2?'five':''"
									...
									<view class="pictrue skeleton-radius">
										<easy-loadimage :image-src="itemn.img" :radius="dataConfig.contentStyle.val*2+'rpx'">
										</easy-loadimage>
									</view>
		...
		<view class="nav oneNav" v-else>
			<scroll-view scroll-x="true" style="white-space: nowrap; display: flex" show-scrollbar="false">
				<block v-for="(item, index) in menus" :key="index">
					<view class="item" v-if="item.status" :style="[titleColor]" @click="menusTap(item.info[1].value)">
						<view class="pictrue skeleton-radius">
							<easy-loadimage :image-src="item.img" :radius="dataConfig.contentStyle.val*2+'rpx'">
							</easy-loadimage>
						</view>
