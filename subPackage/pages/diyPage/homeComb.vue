      ...
      locationConfig() {
        return this.dataConfig.searConfig.tabVal === 2
			...
			currentLocationText() {
				...
				return this.positioningType ? '选择店铺分组' : '选择位置';
			},
        ...
        if(this.dataConfig.searConfig.tabVal< 2) locationManager.clearHistoricalAreaCache()
			...
			initLocationData() {
				try {
					...
					// 备选：使用areas_info（店铺分组选择的位置）
					const areasInfo = uni.getStorageSync('areas_info');
					if (areasInfo && Array.isArray(areasInfo) && areasInfo.length > 0 && this.positioningType) {
						// 显示最新选择的店铺分组
						const latestArea = areasInfo[0];
						this.currentLocationName = latestArea.name || latestArea.address || '选择店铺分组';
						return;
					}
					
					// 默认显示
					this.currentLocationName = this.positioningType ? '选择店铺分组' : '选择位置';
				} catch (error) {
					this.currentLocationName = this.positioningType ? '选择店铺分组' : '选择位置';
				}
			},
			...
			checkLocationUpdate() {
				try {
					let newLocationName = this.positioningType ? '选择店铺分组' : '选择位置';
					...
					if (locationInfo && (locationInfo.name || locationInfo.address)) {
						...
						if (areasInfo && Array.isArray(areasInfo) && areasInfo.length > 0) {
							const latestArea = areasInfo[0];
							newLocationName = latestArea.name || latestArea.address || '选择店铺分组';
						}
					}
			...
			async checkLocationPermissionAndNavigate() {
				try {
					...
					let isAgreePopup = true
					let hasPermission = false
					
					// #ifdef APP-PLUS
					if (!isIos) {
						// 检查当前权限状态
						const result = await authorizeUtils.showAuthTipModal('android.permission.ACCESS_FINE_LOCATION');
						hasPermission = result.hasPermission
						isAgreePopup = result.isAgreePopup
					} else {
						hasPermission = await locationManager.hasPermission();
					}
					// #endif
