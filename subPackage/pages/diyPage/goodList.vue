	...
	export default {
      ...
      regionIds: {
		...
		computed: {
			...
			boxStyle() {
				// 根据展示样式计算每个商品的高度
				// 三列比两列低三分之一：240 * 2/3 = 160
				let itemHeight = 240; // 单列、两列
				if (this.itemStyle == 2) itemHeight = 145; // 三列（比两列低1/3）
				if (this.itemStyle == 3) itemHeight = 890; // 大图

				// 使用实际商品数量计算高度，如果没有数据则给一个默认最小高度
				const count = this.tempArr.length || this.numConfig || 1;
				const estimatedHeight = (count * itemHeight) + 'rpx';
