<template>
	...
	<view class="title-box acea-row row-between row-middle" :style="[...boxStyle]" @click="goPage">
		...
		<view v-if="!selectShow" :style="[...titleRightStyle]">{{ titleRightTxt }}<text :style="[...titleRightStyle]"  class="iconfont icon-ic_rightarrow"></text></view>
	</view>
</template>
	...
	export default {
      ...
      merId: { // 商户id
		...
		methods: {
			...
			goPage() {
        let path = this.$util.appendMerIdToLink(this.dataConfig.linkConfig.val, this.merId, false)
