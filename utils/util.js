...
import { uploadBase64Api } from "../api/api";
...
export default {
	 ...
	 * @returns {Promise} 返回生成的文件路径
	 */
	makeSpreadQRCode(link, canvasId, size = 200, margin = 10) {
	...
	/**
	 * 保存到手机相册
	 * @param posterImage 图片
	 * @returns {Promise<unknown>}
	 */
	savePosterPath(posterImage){
	...
	uploadVideo: function(opt, successCallback, errorCallback) {
		...
		let count = opt.count || 1,
			...
			fils = '',
			uploadMaxSize = 30,
			urlPath = HTTP_REQUEST_URL + '/api/front/upload/file';
			...
			success: async (res) => {
				if (Math.ceil(res.size / 1024) < uploadMaxSize * 1024) {
					...
					video.addEventListener('loadeddata', async function() {
						...
						let { data } = await uploadBase64Api({base64Url: dataURL});
						uni.hideLoading();
						let coverURL = data.url
	...
	uploadImageOne: function(opt, successCallback, errorCallback) {
		...
		uni.chooseImage({
			...
			success: async (res) => {
				...
				let imageList = [];
				let urlPath = HTTP_REQUEST_URL + '/api/front/upload/image';
				if (count === 1) {
	...
	navigateTo(url) {
		if (url.indexOf("http") !== -1) {
			...
			if (['/pages/goods_cate/index', '/pages/order_addcart/order_addcart', '/pages/user/index',
				...
				store.commit('Change_Advertisement', false);
				uni.switchTab({
					animationType: animationType.type,
					animationDuration: animationType.duration,
					url: url
	 ...
	 * @returns {number} 距离（米）
	 */
	getDistanceFromLatLonInMeter(lat1, lng1, lat2, lng2) {
  function toRad(d) { return d * Math.PI / 180; }
  const R = 6378137; // WGS84标准地球半径，单位米
  ...
  return Math.round(distance);
	...
	// 去店铺首页
	goToMerHome(merId){
		this.navigateTo(`/pages/merchant/home/index?merId=${merId}`)
	},
	// 拼接merId参数到链接, isMerFooter 是否使用配置的底部导航
	appendMerIdToLink(linkUrl, merId, isMerFooter) {
		if (!linkUrl) return '';
		let result = linkUrl;
		// 检查是否包含small_page
		if (linkUrl.includes("small_page") && isMerFooter) {
			result = result.includes('?') ? `${result}&fromTo=merHome` : `${result}?fromTo=merHome`;
		}
		// 拼接merId参数
		result = result.includes('?') ? `${result}&merId=${merId}` : `${result}?merId=${merId}`;
		return result;
	},
}