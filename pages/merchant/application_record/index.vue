...
<script>
	...
	export default {
		...
		onLoad(options) {
			options = options || {};
			this.pageData.applyType = options.applyType || options.type || 'store';
			if (this.pageData.applyType !== 'business') this.pageData.applyType = 'store';
			uni.setNavigationBarTitle({
				title: `${this.pageData.applyType === 'business' ? '商户' : '店铺'}入驻记录`
			});
			this.getListData()
		},
		// 滚动到底部
		...
		methods: {
			goDetal(item) {
				...
				uni.navigateTo({
					url: `/pages/users/settled/index?key=1&applyType=${item.applyType || this.pageData.applyType}`
				})
			},
			returns() {
				uni.navigateBack()
			...
			menusTap() {
				uni.navigateTo({
					url: `/pages/users/settled/index?applyType=${this.pageData.applyType}`
				})
			},
			getListData() {
				this.loading = true
				...
				getMerSettledRecordApi(this.pageData).then(res => {
					...
					res.data.list.map(item => {
						let qualificationPicture = item.qualificationPicture;
						if (typeof qualificationPicture === 'string' && qualificationPicture) {
							qualificationPicture = JSON.parse(qualificationPicture);
						}
						this.$set(item, 'qualificationPicture', qualificationPicture || []);
					})
