      ...
      couponDialog
		...
		data() {
			return {
				...
				componentData: {},
				retryTimer: null, // iOS首次启动网络权限弹窗重试定时器
			}
		},
		//下拉刷新
		onPullDownRefresh() {
		...
		methods: {
			...
			initRegionIds() {
				try {
					...
					console.error('页面获取店铺分组ID失败:', error);
					this.regionIds = '';
				}
			},
			...
			getDiyData(diyId, diyType) {
				...
				getDiy({
					...
					uni.hideLoading();
					// #ifdef APP-PLUS
					// iOS首次启动网络权限弹窗兼容：若componentData为空，1.5s后自动重试一次
					if (!this.componentData || !this.componentData.value) {
						clearTimeout(this.retryTimer);
						this.retryTimer = setTimeout(() => {
							this.getDiyData(diyId, diyType);
						}, 1500);
					}
					// #endif
