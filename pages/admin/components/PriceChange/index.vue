		...
		<view class="priceChange" :class="[change === true ? 'on' : '']">
			<view class="priceChange-box">
				<view class="priceTitle">
					{{
					status == 7?'退款单备注':status == 8?'退款原因': status == 0?"一键改价": status == 1?`${title}备注`: isRefund==8?'确认收货':'退款审核'
        }}
          <CloseIcon @handle-close="close" topStyle="top: 36rpx"></CloseIcon>
				</view>
				<!-- 一键改价 -->
					...
					<view class="item acea-row row-between-wrapper">
						...
						<view class="money acea-row row-right">
							<view class="radio-item acea-row row-middle" :class="{ on: isAgree }"
								...
								<text class="iconfont"
									:class="isAgree?'icon-a-ic_CompleteSelect':'icon-ic_unselect'"></text>同意退{{isRefund?'款':'货'}}
							</view>
							<view v-if="orderInfo.refund_type != 4 && orderInfo.refund_type != 5"
								...
								<text class="iconfont"
									:class="isAgree?'icon-ic_unselect':'icon-a-ic_CompleteSelect'"></text>拒绝退款
							</view>
						</view>
					</view>
				...
				<view class="listChange" v-if="status == 1||status == 7">
					<textarea placeholder="请填写备注信息..." @input="inputChange(remarkInfo)" v-model="remarkInfo" fixed :cursor-spacing="100"></textarea>
				</view>
...
import {
	...
	export default {
      ...
      title: {
		...
		created() {
			if( this.status && this.status !=='1' )this.getAddressList()
		},
		methods: {
			inputChange(mark){
				this.remarkInfo=mark
			},
