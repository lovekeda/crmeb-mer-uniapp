	...
	<view :class="mode=='pop'?'masks':''" v-show="showBox" @touchmove.stop.prevent="moveHandle">
		<view :class="mode=='pop'?'verifybox':''" style="max-width:90%">
			<view class="verifybox-top" v-if="mode=='pop'">
				请完成安全验证
				<text class="verifybox-close" @click="onClose">
			...
			<view class="verifybox-bottom" :style="{padding:mode=='pop'?'15px':'0'}">
				...
				<view v-if="componentType=='VerifyPoints'">
					<VerifyPoint @success="success" :captchaType="captchaType" :type="verifyType" :figure="figure"
						:arith="arith" :mode="mode" :vSpace="vSpace" :explain="explain" :imgSize="imgSize"
						:blockSize="blockSize" :barSize="barSize" :defaultImg="defaultImg" ref="instance"></VerifyPoint>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	import VerifyPoint from "./verify/verifyPoint.vue"
	...
	export default {
		...
		methods: {
			...
			initStorage() {
				try {
					// 兼容处理：使用同步存储，但添加错误捕获
					let slider = uni.getStorageSync('slider')
					let point = uni.getStorageSync('point')

					// 如果不存在，生成新的UUID
					if (!slider) {
						slider = this.generateUUID('slider')
						uni.setStorageSync('slider', slider)
					}
					if (!point) {
						point = this.generateUUID('point')
