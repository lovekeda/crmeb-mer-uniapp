		...
		<view class="verify-bar-area" :style="{width: imgSize.width,
			...
			<view class="verify-left-bar"
				...
				<view class="verify-move-block"
					...
					<view v-if="type === '2'" class="verify-sub-block" :style="subBlockStyle">
...
<script>
	...
	export default {
		...
		computed: {
			// 计算滑块样式，兼容不同屏幕尺寸
			subBlockStyle() {
				const width = parseInt(this.imgSize.width) || 310
				const height = parseInt(this.imgSize.height) || 155
				return {
					'width': Math.floor(width * 47 / 310) + 'px',
					'height': this.imgSize.height,
					'top': '-' + (height + this.vSpace) + 'px'
				}
			}
		},
		methods: {
			...
			move: function(e) {
				...
				// 使用 Promise 包装 SelectorQuery
				const getBarRect = () => {
					return new Promise((resolve) => {
						const query = uni.createSelectorQuery().in(this)
						query.select('.verify-bar-area').boundingClientRect(data => {
							if (data) {
								resolve({ left: Math.ceil(data.left), width: Math.ceil(data.width) })
							} else {
								// 降级处理：使用默认值
								const imgWidth = parseInt(this.imgSize.width) || 310
								resolve({ left: 0, width: imgWidth })
							}
						}).exec()
					})
				}

				getBarRect().then(({ left: bar_area_left, width: barArea_offsetWidth }) => {
					if (this.status && this.isEnd == false) {
						var move_block_left = x - bar_area_left //小方块相对于父元素的left值
						if (this.type !== '1') { //图片滑动
							const blockWidth = parseInt(this.blockSize.width) || 50
							if (move_block_left >= barArea_offsetWidth - Math.floor(blockWidth / 2) - 2) {
								move_block_left = barArea_offsetWidth - Math.floor(blockWidth / 2) - 2;
							}
						}

						if (move_block_left <= 0) {
							const blockWidth = parseInt(this.blockSize.width) || 50
							move_block_left = Math.floor(blockWidth / 2);
						}

						//拖动后小方块的left值
						const blockWidth = parseInt(this.blockSize.width) || 50
						this.moveBlockLeft = (move_block_left - Math.floor(blockWidth / 2)) + "px"
						this.leftBarWidth = (move_block_left - Math.floor(blockWidth / 2)) + "px"
					}
				})
			},
			...
			//鼠标松开
				...
				if (this.status && this.isEnd == false) {
					if (this.type !== '1') { //图片滑动
						var moveLeftDistance = parseInt((this.moveBlockLeft || '0').replace('px', '')) || 0;
						// 兼容处理：动态计算实际宽度，保留2位小数减少误差
						const imgWidth = parseInt(this.imgSize.width) || 310
						moveLeftDistance = parseFloat((moveLeftDistance * 310 / imgWidth).toFixed(2))
			...
			// 获取存储的clientUid，兼容处理
			getClientUid() {
				let uid = uni.getStorageSync('slider')
				if (!uid) {
					uid = this.generateUUID()
					try {
						uni.setStorageSync('slider', uid)
					} catch (e) {
			...
			getPictrue() {
				...
				const fetchImage = function() {
					...
					getAjcaptcha(data).then((r) => {
						let res = r.data;
						if (res.repCode == '0000') {
							_this.backImgBase = res.repData.originalImageBase64;
							_this.blockBackImgBase = res.repData.jigsawImageBase64
							_this.backToken = res.repData.token;
							_this.secretKey = res.repData.secretKey;
						} else {
							_this.text = res.repMsg;
							// 验证码失效时自动重试
							if (res.repCode == '6201') {
						...
						_this.backImgBase = null
						_this.blockBackImgBase = null
						// 网络错误重试
						if (retryCount < maxRetry) {
							retryCount++
							setTimeout(fetchImage, 1000)
