<template>
	<view style="position: relative">
		<view class="verify-image-out" v-show="showImage">
			<view class="verify-image-panel" :style="{'width': '100%',
		...
		<view class="verify-bar-area" :style="{'width': '100%',
                      'color': barAreaColor,
...
<script type="text/babel">
	...
	import {getAjcaptcha,ajcaptchaCheck} from '../api/index.js';
    ...
    export default {
        ...
        methods: {
            ...
            canvasClick(e) {
                // 直接使用点击坐标（小程序中 e.detail.x 已相对于组件）
                const pos = this.getMousePos(this.$refs.canvas, e)
                this.checkPosArr.push(pos);
                if (this.num == this.checkNum) {
                    this.num = this.createPoint(pos);
                    //按比例转换坐标值
            ...
            getMousePos: function (obj, e) {
                ...
                // #ifdef H5
                const refEl = obj && (obj.$el || obj)
                const eventEl = e && (e.currentTarget || e.target)
                const el = refEl && typeof refEl.getBoundingClientRect === 'function' ? refEl : eventEl
                const pointer = e && ((e.changedTouches && e.changedTouches[0]) || (e.touches && e.touches[0]) || e)
                if (el && typeof el.getBoundingClientRect === 'function' && pointer && typeof pointer.clientX === 'number' && typeof pointer.clientY === 'number') {
                    ...
                    x = e.offsetX
                    y = e.offsetY
                } else if (e && e.detail && typeof e.detail.x === 'number' && typeof e.detail.y === 'number') {
                    x = e.detail.x
                    y = e.detail.y
                }
                // #endif
                ...
                // #ifndef H5
                // 小程序端 - 尝试直接使用 detail 中的坐标
                // e.detail.x 是相对于当前组件的坐标，不需要再减去 imgLeft/imgTop
                // 如果这样不行，再尝试减去 imgLeft/imgTop
                x = e && e.detail && typeof e.detail.x === 'number' ? e.detail.x : 0
                y = e && e.detail && typeof e.detail.y === 'number' ? e.detail.y : 0
                // #endif
            ...
            // 获取存储的clientUid，兼容处理
            getClientUid() {
                let uid = uni.getStorageSync('point')
                if (!uid) {
                    uid = this.generateUUID()
                    try {
                        uni.setStorageSync('point', uid)
                    } catch (e) {
            ...
            getPictrue() {
                ...
                const fetchImage = function() {
                    let data = {
                        ...
                        clientUid: _this.getClientUid(),
                        ts: Date.now(),
                    }
                    getAjcaptcha(data).then((res) => {
                        let data = res.data
                        if (data.repCode == '0000') {
                            _this.pointBackImgBase = data.repData.originalImageBase64;
                            _this.backToken = data.repData.token;
                            _this.secretKey = data.repData.secretKey;
                            _this.poinTextList = data.repData.wordList;
                            _this.text = '请依次点击【' + _this.poinTextList.join(',') + '】';
                        } else {
                            _this.text = data.repMsg;
                            // 验证码失效时自动重试
                            if (data.repCode == '6201') {
                                _this.pointBackImgBase = null;
            ...
            pointTransfrom(pointArr, imgSize) {
                const width = parseInt(imgSize.width) || 310
                const height = parseInt(imgSize.height) || 155
                var newPointArr = pointArr.map(p => {
                    let x = parseFloat((310 * p.x / width).toFixed(2))
                    let y = parseFloat((155 * p.y / height).toFixed(2))
                    return { x, y }
                })
                return newPointArr
            },
