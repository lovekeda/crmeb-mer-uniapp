<template>
	<view :data-theme="theme">
		<view class='newsDetail' :style="{backgroundColor:bgColor}">
			...
			<view class='conter'>
				<!-- 富文本-->
				<rich-text-editor :richTextVal="richText" v-if="richText"></rich-text-editor>
			</view>
			<view class="picTxt acea-row row-between-wrapper" v-if="store_info.id">
				...
				<navigator :url="'/pages/goods/goods_details/index?id='+store_info.id+'&mt=0'" hover-class="none"
					class="label">
...
<script>
	...
	import richTextEditor from "@/subPackage/pages/diyPage/richTextEditor";
	let app = getApp();
	export default {
		components: {
			shareInfo,
