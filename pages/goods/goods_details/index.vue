 	...
 	export default {
        ...
        theme: app.globalData.theme,
 		...
 		methods: {
			 ...
			 */
			appShare: function() {
				let shareUrl = this.getShareH5Url();
				uni.share({
					provider: "weixin",
					scene: "WXSceneSession",
					type: 0,
					title: this.productInfo.name || "商品详情",
					summary: this.productInfo.info || "点击查看详情",
					imageUrl: this.productInfo.image || "",
					href: shareUrl,
					success: function(res) {
			 ...
			 * 获取H5分享链接
			 */
			getShareH5Url: function() {
				let path = "/pages/goods/goods_details/index";
				let params = [];
				...
				// 商品类型
				params.push(`mt=${this.marketingType || 0}`);
				
				// 分享者ID
				if (this.uid) {
					params.push(`sd=${this.uid}`);
				}
				
				// 拼团活动ID
				if (this.marketingType === 2 && this.groupActivityId) {
					params.push(`gd=${this.groupActivityId}`);
				}
				
				// 拼团记录ID
				if (this.groupRecordId) {
					params.push(`rd=${this.groupRecordId}`);
				}
				
				return `${HTTP_H5_URL}${path}?${params.join("&")}`;
			},
